...
| Need | Store |
| --- | --- |
| Implementation config, derived intent evidence, and PR history | GitHub Enterprise |
| Verdict records | PostgreSQL-compatible store |
| Destination registry | PostgreSQL plus graph projection |
| Resolution cache | PostgreSQL or document store with TTL |
| Relationship model | Graph database |
| Runtime inventory and flow history | Object storage, data lake, SIEM, OpenSearch, ClickHouse, or Snowflake |
